    ...
    <header className="sticky top-0 z-40 w-full border-b border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
          ...
          {/* زر البحث في السجل الطبي للمريض برقم الهاتف */}
          <button
            onClick={() => setPatientHistoryModalOpen(true)}
            className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-emerald-700 dark:hover:text-emerald-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200/80 dark:hover:bg-slate-700/80 rounded-lg transition-all"
            title="البحث عن تذاكر وحجوزات المريض السابقة"
          >
            <Search className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-600 dark:text-emerald-400" />
            <span className="hidden sm:inline">سجل المريض</span>
          </button>
