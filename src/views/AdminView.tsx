...
import { DailyClinicScheduleItem, SystemPermission, UserRole, Doctor } from '../types';
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-base text-slate-900 dark:text-white">
                    أمان الحسابات وتعيين كلمات المرور للكادر (Staff Credential Security)
                  </h3>
                </div>
                <p className="text-xs text-slate-500 mt-1">
                  تشفير SHA-256 مشدد مع حماية تلقائية ضد هجمات القوة العمياء والتخمين (Brute-Force Lock). يمكن للمدير تعيين كلمة مرور جديدة فوراً لأي مستخدم.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 pt-2">
              {[
                { roleName: 'مدير المنظومة', user: 'admin', icon: LayoutDashboard, color: 'text-emerald-600' },
                { roleName: 'مسؤول الاستقبال', user: 'reception', icon: UserCheck, color: 'text-amber-600' },
                { roleName: 'أمين الخزينة', user: 'cashier', icon: Receipt, color: 'text-teal-600' },
                { roleName: 'طبيب العيادة', user: 'doctor', icon: Stethoscope, color: 'text-blue-600' },
              ].map(acc => {
                const IconComponent = acc.icon;
                return (
                  <div key={acc.user} className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center">
                        <IconComponent className={`w-4 h-4 ${acc.color}`} />
                      </div>
                      <div>
                        <div className="text-xs font-bold text-slate-900 dark:text-white">{acc.roleName}</div>
                        <div className="text-[10px] font-mono text-slate-400">@{acc.user}</div>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        setResetTargetUser(acc.user);
                        setNewStaffPassword('');
                        setShowResetPassModal(true);
                      }}
                      className="px-2.5 py-1 text-[11px] font-semibold text-emerald-700 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 rounded-lg transition-colors cursor-pointer"
                    >
                      تغيير الرمز
                    </button>
                  </div>
